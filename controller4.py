...
class VantaController:
    ...
    def openflow_handshake(self, sock):
        ...
        try:
            ...
            # SEND HELLO FIRST with version bitmap (OpenFlow 1.3+ requirement)
            self.log(">>> Sending HELLO to switch...", "HAND")
            
            # Build HELLO with version bitmap element
            # Type=1 (OFPHET_VERSIONBITMAP), Length=8, Bitmaps for versions 1.0-1.6
            hello_elem = struct.pack('!HHI',
                                    1,      # type: OFPHET_VERSIONBITMAP
                                    8,      # length
                                    0x7F)   # bitmap: support versions 1-6 (bits 1-6 set)
            
            hello_msg = struct.pack('!BBHI', OFP_VERSION, OFPT_HELLO, 8 + len(hello_elem), 1)
            hello_msg += hello_elem
            
            sock.send(hello_msg)
            self.log("✓ Sent HELLO with version bitmap (supporting OF 1.0-1.6)", "HAND")
